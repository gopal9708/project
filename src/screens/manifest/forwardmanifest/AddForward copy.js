...
const AddForward = (data) => {
  ...
  // data.manifest
  useEffect(() => {
    if (location_data.state.manifest) {
      setmanifest_no(location_data.state.manifest.manifest_no);
      setforward_branch(
        toTitleCase(location_data.state.manifest.from_branch_n)
      );
      setorgin(toTitleCase(location_data.state.manifest.orgin_branch_n));
      setdest(toTitleCase(location_data.state.manifest.destination_branch_n));
      setmanifest_id(location_data.state.manifest.id);
    }
  }, []);
  ...
  return (
    <>
      ...
      <Modal show={show} onHide={handleClose} contentClassName="content-test">
        ...
        >
          <div className="mt-3">
            <PageTitle page={"Create Manifest"} />
            <Title title={"Create Manifest"} parent_title="Manifest" />
          </div>
