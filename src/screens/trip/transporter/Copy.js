...
const AddTransporter = () => {
  ...
  const getStates = () => {
    let state_list = [];
    axios
      .get(ServerAddress + "master/all-states/", {
      ...
      .catch((err) => {
        alert(`Error Occur in Get  ${List_Name} , ${err}`);
      });
  };
  ...
  const getLocations = (place_id, filter_by, city_type) => {
    ...
    axios
      .get(
        ServerAddress +
          `master/all-locations/?place_id=${place_id}&filter_by=${filter_by}`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      )
