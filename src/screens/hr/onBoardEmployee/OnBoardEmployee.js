...
const OnBoardEmployee = () => {
  ...
  useEffect(() => {  
    if (
      userpermission.some((e) => e.sub_model === "OnBoard Employee" && e.delete === true)
    ) {
      setcan_delete(true);
    } else {
      setcan_delete(false);
  ...
  return (
    <>
      <PageTitle page="Onboard Employee" />
      <Title title="Onboard Employee" parent_title="EMS" />
      <div className="mx-3">
        <div className="container-fluid" style={{ background: "white" }}>
          ...
          can_delete={can_delete}
            Data_Title={OnBordingDataTitle}
            Data_Format={OnBoardingDataFormate}
            path={`ems/get_notonboardedlist/?onboarded="incomplete"&?search=${search}&p=${page_num}&records=${data_len}`}
          />
