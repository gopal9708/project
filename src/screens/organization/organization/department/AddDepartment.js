...
function AddDepartment() {
  ...
  const send_department_data = (values) => {
    axios
      .post(
        ServerAddress + "organization/add_department/",
        {
      ...
      .then(function (response) {
        console.log("send DEPTT data response==",response);
        if (response.data.status === "success") {
          add_user_permission(response.data.data.id);
          dispatch(setToggle(true));
  ...
  const changed_department_role = (values) => {
    axios
      .put(
        ServerAddress + "organization/update_department/" + departments.id,
        {
          name: String(values.department).toUpperCase(),
          // priority: values.priority,
          // is_checker_maker: is_checker_maker,
