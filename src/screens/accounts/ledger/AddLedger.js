...
const AddLedger = () => {
  ...
  // console.log("main group", main_gp_id);
  // console.log("sub group 1", sub_group_id);
  // console.log("sub group 2", sub_group_2_id);

  useEffect(() => {
    try {
      // setupdate_is_list(location.state.item.group_name, location.state.item?.subgroup_name1);
      setledger_id(location.state.item?.id);
      setledger_name(location.state.item?.name);
      setis_updating(true);

      setget_group_data(location.state.item.group_name);
      setmain_group(location.state.item?.group_name);
      setmain_gp_id(location.state.item?.group);

      setsub_group(location.state.item?.subgroup_name1);
      setsub_group_id(location.state.item?.sub_group1);

      setsub_group_1(location.state.item?.subgroup_name2);
      setsub_group_1_id(location.state.item?.sub_group2);

      setsub_group_2(location.state.item?.subgroup_name3);
      setsub_group_2_id(location.state.item?.sub_group3);
    } catch (error) {}
  }, []);
  ...
  }, [get_group_data, sub_group, sub_group_1, sub_group_2]);
  ...
  const add_group = (type, value) => {
    axios
      .post(
        ...
        {
          group_type: type,
          group_name: toTitleCase(value).toUpperCase(),
        },
        {
          headers: { Authorization: `Bearer ${AccessToken}` },
        }
  ...
  const add_subgroup = (value, id) => {
    axios
      ...
      .then((resp) => {
        ...
        if(resp.data.data  === "Data Already Exists") {
          alert( `${value} ${resp.data.data}`);
        }
        if (resp.status === 201) {
          setadd_new(false);
          get_main_group();
  ...
  const add_ledger = () => {
    axios
      .post(
        ...
        {
          name: toTitleCase(ledger_name).toUpperCase(),
          group_ids: id_list,
          main_group: is_main,
        },
        {
  ...
  useEffect(() => {
    if ( main_gp_id !== "" && is_main_group === "NO") {
      get_sub_group("level1", main_gp_id);
    }
  }, [main_gp_id, refresh]);

  console.log("is_main_group----", is_main_group)
  console.log("is_main_group_1----", is_main_group_1)
  console.log("is_main_group_2---", is_main_group_2)

  console.log("main_group---", main_group)
  console.log("sub_group---", sub_group)
  console.log("sub_group_1---", sub_group_1)
  console.log("sub_group_2---", sub_group_2) 

  console.log("id_list---", id_list)

  useEffect(() => {
    if (sub_group_id !== ""  && is_main_group_1 === "NO"
    // && is_main_group === "NO"
    ) {
  ...
  // for validation
  const [ledger_name_error, setledger_name_error] = useState(false);
  const [main_group_error, setmain_group_error] = useState(false);
  const [sub_group_error, setsub_group_error] = useState(false);
  const [sub_group_1_error, setsub_group_1_error] = useState(false);
  const [sub_group_2_error, setsub_group_2_error] = useState(false);

  useEffect(() => {
    if (ledger_name !== "") {
      setledger_name_error(false);
    }
  }, [ledger_name]);
    ...
    <div>
      <Form
        onSubmit={(e) => {
          e.preventDefault();
          if (ledger_name === "") {
            setledger_name_error(true);
          } 
          // else if (main_group === "") {
          //   setmain_group_error(true);
          // } else if (is_main_group === "") {
          //   alert("Please Select Any Option");
          // } else if (is_main_group === "NO" && sub_group === "") {
          //   setsub_group_error(true);
          // } else if (is_main_group_1 === "") {
          //   alert("Please Select Any Option");
          // } else if (is_main_group_1 === "NO" && sub_group_1 === "") {
          //   setsub_group_1_error(true);
          // } else if (is_main_group_2 === "") {
          //   alert("Please Select Any Option");
          // } else if (is_main_group_2 === "NO" && sub_group_2 === "") {
          //   setsub_group_2_error(true);
          // } 
          // else if (is_main_group_3 === "") {
          //   alert("Please Select Any Option");
          // } 
          else {
                        ...
                        <div className="m-2 mb-2">
                          ...
                          <div style={{ display: "flex" }}>
                            <NSearchInput
                              ...
                              current_width="240px"
                              error_message={"Please Select Sub Group"}
                              error_s={sub_group_error}
                            />
<span>
                            <AiFillPlusCircle
                              onClick={() => {
