...
const AddCsvn = () => {
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      ...
      send_payroll_data(values);
    },
  });
  // post method
  const send_payroll_data = (values) => {
    axios
      .post(
        ServerAddress + "",
        {
      ...
      .then((response) => {
        if (response.data.status === "success") {
          ...
          dispatch(
            setDataExist(` Payroll ${values.branch} is Added Successfully`)
          );
          navigate(-1);
        }
      })
    ...
    <div>
      ...
      >
        <div className="m-4">
          <PageTitle
            page={
              is_updating ? "Update PayRoll Records" : "Add PayRoll Records"
            }
          />
          <Title
            title={
            ...
            {circle_btn ? (
              <CardBody>
                <Row>
                  <Col lg={4} md={6} sm={6}>
                    <div className="mb-3">
                      <Label className="header-child">Archieved</Label>
                  ...
                  <Col lg={4} md={6} sm={6}>
                    <div className="m-2 mb-2">
                      <Label className="header-child sm-10">
                        Month <span className="mandatory">*</span> :
                      </Label>
                      <Input
                        className="form-control d-block from control-md"
                        bsSize="sm"
