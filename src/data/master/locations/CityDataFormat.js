...
const CityDataFormat = ({ data, data1, can_delete }) => {
  ...
  const deleteCity = (id) => {
    axios
      .post(
        ServerAddress + "master/delete_city/",
        {
          data: id,
        },
        {
      ...
      .catch((err) => {
        ...
        dispatch(
          setDataExist(
            `City Already Exists Some Where`
          )
        );
  ...
  return (
    <>
      {(list_toggle === true ? data1 : data).length === 0 ? (
        ...
        (list_toggle === true ? data1 : data).map((city, index) => {
          return (
            <tr
              key={index}
              style={{
            ...
            >
              ...
              <td>{toTitleCase(city.state_name)}</td>
            </tr>
          );
        })
      )}
