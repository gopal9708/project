...
const VoucherDataFormate = ({ data, data1 }) => {
        ...
        (list_toggle === true ? data1 : data).map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <td>
                {/* {
                  user_permissions.includes("Can change Branch") ? */}
                <Link to="/accounts/voucher/AddVoucher" state={{ item: item }}>
                  {toTitleCase(item.voucher_no)}
                </Link>
              </td>
              <td>{item.date}</td>
...
export default VoucherDataFormate;