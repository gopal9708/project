...
const JobAdvDataFormate = ({ data, data1, can_delete, }) => {
  console.log("xyzzzz", data);
  ...
  const delete_select_item = (id) => {
    axios
      .post(
        ServerAddress + "ems/delete_job_advmnt/",
        {
          data: id,
        },
        {
            ...
            return (
              <tr key={index} style={{ borderWidth: 1 }}>
                ...
                <td>
                  ...
                  <Link
                    state={{ data: item }}
                    to="/hr/recruitment/jobAdvertisements/JobAdvertisements"
                  >
                    {item.position}
                  </Link>
                  ) : (
...
export default JobAdvDataFormate;
