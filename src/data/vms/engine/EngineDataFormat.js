...
const EngineDataFormat = ({ data }) => {

  console.log("Engine Data Formate Data is==", data);
  ...
  return (
    <>
      {data.length == 0 ? (
        ...
        <>
          {data.map((item, index) => {
            console.log("return Engine data Formate Data is==", item);
            return (
              <>
                <tr
                  key={index}
                ...
                >
                  ...
                  <Link to="/vehicleEngine/AddVehicleEngine"
                    state={{ item: item }}>
                    <td>{toTitleCase(item.vehicle_model_nm)}</td>
                  </Link>
                    <td>{item.engine_summary}</td>
                  <td>{toTitleCase(item.engine_brand)}</td>
                  <td>{toTitleCase(item.aspiration)}</td>
                  <td>{toTitleCase(item.block_type)}</td>
                  <td>{toTitleCase(item.cam_type)}</td>
...
export default EngineDataFormat;
