...
const CardMasterDataformate = ({ data, data1, can_delete }) => {
  ...
  const delete_select_item = () => {
    axios
      .post(
        ServerAddress + "finance/delete-CardMasterEntry/",
        {
          data: ids,
        },
        {
  ...
  useEffect(()=> {
    if (
      userpermission.some((e) => e.sub_model === "Card Master Entry" && e.update === true)
    ) {
      setcan_update(true);
    } else {
      setcan_update(false);
            ...
            return (
              <tr key={index} style={{ borderWidth: 1 }}>
                ...
                <td>
                  ...
                  <Link
                    state={{ data: item }}
                    to="/master/cardmaster/addcardmaster"
                  >
                    {item.from_branch}
                  </Link>
                  ) : (
...
export default CardMasterDataformate;
