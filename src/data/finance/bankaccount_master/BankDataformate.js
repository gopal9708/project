...
const BankDataformate = ({ data, data1, can_delete }) => {
  ...
  const delete_select_item = () => {
    axios
      .post(
        ServerAddress + "finance/delete-BankAccountMaster/",
        {
          data: ids,
        },
        {
  ...
  useEffect(()=> {
    if (
      userpermission.some((e) => e.sub_model === "Bank Account Master" && e.update === true)
    ) {
      setcan_update(true);
    } else {
      setcan_update(false);
            ...
            return (
              <tr key={index} style={{ borderWidth: 1 }}>
                ...
                <td>
                  ...
                  <Link
                    state={{ data: item }}
                    to="/master/bankaccountmaster/addbankaccountmaster"
                  >
                    {item.branch}
                  </Link>
                  ) : (
...
export default BankDataformate;
