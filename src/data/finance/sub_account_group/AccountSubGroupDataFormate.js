...
const AccountSubGroupDataFormate = ({ data, data1, can_delete }) => {
  ...
  const delete_select_item = () => {
    axios
      .post(
        ServerAddress + "finance/delete-AccountSubGroupMaster/",
        {
          data: ids,
        },
        {
  ...
  useEffect(()=> {
    if (
      userpermission.some((e) => e.sub_model === "Account Sub Group Master" && e.update === true)
    ) {
      setcan_update(true);
    } else {
      setcan_update(false);
            ...
            return (
              <tr key={index} style={{ borderWidth: 1 }}>
                ...
                <td>
                  ...
                  <Link
                    state={{ data: item }}
                    to="/master/accountsubgroupmaster/addaccountsubgroupmaster"
                  >
                    {item.sub_group_code}
                  </Link>
                  ) : (
...
export default AccountSubGroupDataFormate;
